define(["config"],function(config) {
	...
	var TrackBuffer = function(options) {
		...
		this.ct = options.ct;
		this.simplifyTracks = true;
		this.simplifyTracksPrecision = config.pxTrackPrecision;
		this.lines = {};
		this.tiles = {};
		this.newTilesIndex = {};
		this.zoom = 0;
						...
						for (var xI = Math.min(nn.x,nnPrev.x), xN = Math.max(nn.x,nnPrev.x); xI <= xN; xI++) {
							...
							for (var yI = Math.min(nn.y,nnPrev.y), yN = Math.max(nn.y,nnPrev.y); yI <= yN; yI++) {
								...
								if (!tiles2fill[xI+"_"+yI]) {
									t.context.strokeStyle = line.color;
									t.context.beginPath();
	...
	TrackBuffer.prototype.reset = function() {
		for (var i in this.tiles) {
			if (this.tiles.hasOwnProperty(i)) {
				this.tiles[i].canvas.height = this.tiles[i].canvas.height;
			}
		}
		this.lines = {};
	}
